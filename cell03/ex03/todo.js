...
const getTodos = ()=>{ 
  try{ 
    return JSON.parse(decodeURIComponent(document.cookie.replace(/(?:(?:^|.*;\s*)todo_list\s*=\s*([^;]*).*$)|^.*$/,"$1")))||[];
  }catch(e){ return[]; } 
};
...
const updateEmptyMessage = ()=>{ emptyMsg.hidden = list.querySelectorAll('.todo_item').length > 0; }
...
const addTodo = text=>{
  ...
  div.onclick = ()=>{
    if(confirm('Delete this TO DO?')){
      div.remove(); saveTodos(); updateEmptyMessage();
    }
  };
  list.prepend(div);
  saveTodos();
